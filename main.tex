...
the profinite completion $\Zhat$, where the survivors have a Haar
measure $\nu$ that always exists, and prove unconditionally that
the upper logarithmic density of $A$ is at most $\nu$, so that
the reverse inequality would settle the problem.  We prove the
reverse inequality in a series of regimes -- convergent drift,
incompatible, coprime, aligned and boundedly misaligned systems,
cylinder decompositions, and three reduction theorems for
systems organized along protectors -- and give a second-moment
criterion for $\nu=0$.  We exhibit the step of the
Davenport--Erd\H{o}s argument that shifted residues break, refute
four natural routes by explicit construction, and state what
remains as a checklist a counterexample must pass.  The problem
...
convergent drift, incompatible systems, systems aligned to one
non-negative integer, and finitely many aligned channels (Theorems~\ref{thm:conv},
...
Appendix~\ref{sec:verification} describes the verification.  We emphasize at the
outset that the problem is \emph{not} resolved here.

\paragraph{The problem in the literature.}
...
excluded modulo each $n$; its statement has been formalized in
...
neighboring question, Problem~281, asks: if $n_1<n_2<\cdots$ is
...
$\varepsilon$?  It was answered affirmatively in January 2026 in a blog post
of Tao~\cite{TaoRogers}, observing that an old theorem of Rogers
...
where the neighboring literature stops.  The lower bounds on
...
profinite integers with normalized Haar measure $\mu$.
...
A \emph{cylinder} $c+d\mathbb{Z}$ is a residue class of
integers; its \emph{ball} is $c+d\Zhat$, and $\mu(C)$ for a
cylinder $C$ always means the Haar measure of its ball, so that
statements mixing $\mu(C)$ with integer densities on $C$ refer to
the ball and the residue class respectively.  Passing to a
cylinder, as we shall do repeatedly, changes the
...
its shape with a larger constant.  And a class whose kills on a cylinder are, apart from
finitely many integers, the multiples of some $m$ there, keeps
that description under a threshold: the threshold changes the
kills only below the modulus, and the arguments that use this
description (Theorem~\ref{thm:bounded}) need it only above a
finite bound.  Fourth, a class of modulus $n$ lying inside a cylinder
...
result of this paper about integer densities is stated for
standard systems and holds for threshold systems with the same
proof, thresholds entering
only through these five facts: Theorems~\ref{thm:upper}
and~\ref{thm:D} through the first, Theorem~\ref{thm:conv} through the second,
...
stated for standard systems only.  Its proof uses that a class
kills nothing below its modulus; a threshold class inside a
maximal null cylinder of modulus $d$ may kill the one integer of
the cylinder's residue class below $d$, and we have not
determined whether these extra kills can carry positive
logarithmic density.  The theorem is applied only to the original
system.  Induced systems arise in two
...
the inequality, and the reduction theorems of
Section~\ref{sec:reductions} and the criteria of
Section~\ref{sec:reduction} are stated for its conclusion,
$\dlog(A)=\nu$.

\section{Two Lemmas on Sets of Multiples}\label{sec:lemmas}

We isolate two facts: the first is used in
Sections~\ref{sec:regimes} and~\ref{sec:cyl}, the second only
in Remark~\ref{rem:gap} of Section~\ref{sec:cyl}.  Write $M(D)$
...
be normalized to be nonnegative.  Theorem~\ref{thm:chan} is
...
misaligned parts are the small primes $q$ (apart from the aligned
classes at the positions $j\equiv0\pmod q$) and so are unbounded,
and for any $M$ the classes excluded from
Corollary~\ref{cor:mixed} include those with $q\nmid M$ at the
other positions, whose drift is at least
$\sum_{q\nmid M}\bigl((1-1/q)S_q/q-1/\min P_q\bigr)$ and diverges by
Mertens -- so neither
...
hypothesis and the survivors vanish.
...
\begin{remark}
The aligned case -- all $c_j\equiv c\pmod{d_j}$ for one integer
...
\begin{remark}\label{rem:manyprot}
...
prime cover, since for any finite set $F$ of primes some $p_j$
lies outside $F$ and $m$ may be taken a prime outside $F$; the
...
three cylinders measured the survivor share is $0.0000$, and outside
all six protectors $1.0000$.  So positive survivor measure against
...
over all $m\ge2$ coprime to $q_i$ -- a misaligned family of
cylinders, $172$ of them in the truncation measured -- and let
each be exhausted by its own group of level-zero classes.  The
...
saturated system majorizes the original, and a density for it
...
\begin{proof}
The profinite unions of the systems $G_1\cup\dots\cup G_J$ increase to that of
the full system, so $\nu_J\downarrow\nu$, and by hypothesis
...
non-trivial dying cylinder, and this theorem covers it.
\end{remark}
...
of its proper ancestors.  The \emph{cells} of depth $D$ are the
nodes of depth $D$ together with the residue classes composing
the gaps of nodes of smaller depth; they partition $\Zhat$, and
the partitions refine as $D$ grows.  A system is \emph{coprime-layered} on
$T$ if
\begin{enumerate}
...
Theorem~\ref{thm:conv} their survivor set $A_L$ has logarithmic
...
Hence $A\supseteq A_L\setminus\bigcup_j(C_j\cap[d_j/M',\infty))$,
...
partition of $\Zhat$ into the cells of depth $D$; each cell is a
single residue class, and the partitions refine as $D$ grows,
since a gap once formed is never subdivided.  The dying set $E=\{g=\infty\}$ is determined
...
forbids; and a root class cannot contain a class attached to a
descendant for the same reason, its ball lying inside $C$.  Its branch drifts are those of the original along the
...
theorem.  It would not: the lemma's analog for composite layers
...
$\nu=0.94479$ against a Monte Carlo value of $0.9413$.  Since
$\sum_{P<q\le x}1/q\approx\log(\log x/\log P)$, a layer of drift
$S$ drawn from distinct primes above $P$ reaches primes near
$P^{e^S}$, so layers of drift $1$ exhaust any finite pool at
once and the instance checks the formula and the split, not
...
covers it.
Suppose then that $H:=\sum_{p\in\mathcal{S}}1/p=\infty$, and let
...
=1/(pp'p'')$.  Writing $H_N,B_N$ for $\sum1/p$ and $\sum1/p^2$
over the first $N$ primes of $\mathcal{S}$ above $P$, and
$M_N=(H_N^2-B_N)/2$ for the total measure of the cylinders,
\[
\sum_{e,f}\mu(C_e\cap C_f)=M_N+\sum_p\frac1p\Bigl[(H_N-1/p)^2-(B_N-1/p^2)\Bigr]
...
$p^{-1}[(H_N-1/p)^2-(B_N-1/p^2)]$; summing over $p$ gives the
...
pairs, $\Sigma'=\sum_e\mu(C_e)^2+\sum_pp^{-2}[(H_N-1/p)^2-(B_N-1/p^2)]$.
...
most $H_N^3$, $\Sigma'\le B_N^2+H_N^2B_N$, and $M_N\sim H_N^2/2$,
so the displayed sum is $M_N^2(1+O(1/H_N))$, and the
second-moment ratio of the kills is at most
$\kappa_0^{-2}(1+O(1/H_N))$.  So
...
$H_N\asymp\log\log N$: over all odd primes up to
$10^3,10^4,10^5,10^6,10^7$ the exact ratios are 3.72, 3.29, 3.03, 2.87, 2.74, while the
normalized quantity $(R-1)H/4$ falls through 1.1553, 1.1345, 1.1217, 1.1129, 1.1065 toward the
...
$\{X:\ \sigma(X)>\delta\}$ has upper logarithmic density at most
...
being $1.000136$; by contrast \emph{sub}-systems -- the primes in
$(X^{1/20},X^{1/2}]$ at residue $0$, in the same program -- reach
$1.3297$, and those of Remark~\ref{rem:intermediate} reach
$1.7388$, so the
...
fails: the many-protector system of Remark~\ref{rem:manyprot}
...
no proper exhaustion step on any cylinder of modulus at least
$2$, and Theorem~\ref{thm:quasi} settles it.
...
in the sense of Theorem~\ref{thm:exhaust}, and on cylinders of
modulus at least $2$ the system admits no proper exhaustion step,
only the trivial ones in which a class is its own group (on
$\Zhat$ itself the whole system is a dying group, by (iii));
...
$\sum_{j\le6}1/p_j=0.1999$; every class is misaligned, $6{,}490$
of them with misaligned part the whole modulus, and the misaligned
parts grow without bound.
...
restriction: any two class-carrying nodes other than the root
must have moduli sharing a factor, so the systems it reaches are
those whose classes hang off a family of pairwise non-coprime
cylinders.  Two natural
...
arbitrary system.  Each theorem assumes a given organization of
the classes into groups with a stated relation between their
moduli, and no result here produces such an organization from
nothing.  That is
too generous to the organization theorems, whose hypotheses are
...
any organization at all.  The final form is therefore the
...
for this test: its drift grows like $s\log\log X$ and, in the computed instance
with $Q$ the odd primes and blocks for the eight $q\le23$,
reaches only $0.05$, so neither the protected nor the
...
near $1-\prod_{N\le n\le2N}(1-1/n)\to\tfrac12$: Monte Carlo
estimates of the Haar measure on $\Zhat$, from $20{,}000$ points
each, give $0.516$, $0.513$ and $0.507$ at $N=20,40,80$ with
random residues, against $0.426$, $0.413$ and $0.392$ for the
same moduli aligned at $0$.  So if such a block lies beyond the head
...
blocks confined to a common protector fall under
Theorem~\ref{thm:exhaust} when they exhaust it, and otherwise are
the open case once more.  What is left is a sequence of blocks
...
the residue $\equiv j\pmod q$ and $\equiv0\pmod{p_j}$, so that its
misaligned part is $q$ except at the positions $j\equiv0\pmod q$,
where the class is aligned, and the residue groups are the
...
Rohrbach \cite{Heilbronn,Rohrbach}, generalized by Behrend
...
The programs behind every computation reported in this paper,
their recorded outputs, and the claims script that reproduces the
values quoted in the text are openly available in the repository
Triarius at \url{https://github.com/ibrahimmian36/Triarius}.
...
a named program of the artifact, whose recorded outputs it
contains.

\begin{thebibliography}{99}

\bibitem{Behrend} Behrend, F. (1948). Generalization of an
inequality of Heilbronn and Rohrbach. Bull. Amer. Math. Soc. 54:
681--684. DOI: 10.1090/S0002-9904-1948-09056-5

\bibitem{Bloom} Bloom, T. F. (2026). Erd\H{o}s Problem \#25.
Available at: \url{https://www.erdosproblems.com/25} (accessed
4 September 2026).

\bibitem{Choj26} Chojecki, P. (2026). Truncated congruence sieves
and Erd\H{o}s Problem 25. Unpublished note, 19 March 2026.
Available at: \url{https://www.ulam.ai/research/erdos25.pdf}

\bibitem{DE1936} Davenport, H., Erd\H{o}s, P. (1936). On sequences
of positive integers. Acta Arith. 2: 147--151. DOI:
10.4064/aa-2-1-147-151

\bibitem{DE1951} Davenport, H., Erd\H{o}s, P. (1951). On sequences
of positive integers. J. Indian Math. Soc. (N.S.) 15: 19--24.

\bibitem{Er36} Erd\H{o}s, P. (1936). A generalization of a theorem
of Besicovitch. J. London Math. Soc. 11: 92--98. DOI:
10.1112/jlms/s1-11.2.92

\bibitem{Er48} Erd\H{o}s, P. (1948). On the density of some
sequences of integers. Bull. Amer. Math. Soc. 54: 685--692. DOI:
10.1090/S0002-9904-1948-09057-7

\bibitem{FFKPY} Filaseta, M., Ford, K., Konyagin, S., Pomerance,
C., Yu, G. (2007). Sieving by large integers and covering systems
of congruences. J. Amer. Math. Soc. 20: 495--517. DOI:
10.1090/S0894-0347-06-00549-2

\bibitem{HaRo66} Halberstam, H., Roth, K. F. (1966).
\emph{Sequences}, Vol.~I. Oxford, UK: Clarendon Press.

\bibitem{Hall} Hall, R. R. (1996). \emph{Sets of Multiples}.
Cambridge Tracts in Mathematics, Vol.~118. Cambridge, UK:
Cambridge University Press. DOI: 10.1017/CBO9780511566011

\bibitem{HT92} Hall, R. R., Tenenbaum, G. (1992). On Behrend
sequences. Math. Proc. Cambridge Philos. Soc. 112: 467--482. DOI:
10.1017/S0305004100071140

\bibitem{Heilbronn} Heilbronn, H. (1937). On an inequality in the
elementary theory of numbers. Proc. Cambridge Philos. Soc. 33:
207--209. DOI: 10.1017/S030500410001954X

\bibitem{Hough} Hough, R. (2015). Solution of the minimum modulus
problem for covering systems. Ann. of Math. (2) 181: 361--382.
DOI: 10.4007/annals.2015.181.1.6

\bibitem{KS} Kochen, S., Stone, C. (1964). A note on the
Borel--Cantelli lemma. Illinois J. Math. 8: 248--251. DOI:
10.1215/ijm/1256059668

\bibitem{Rohrbach} Rohrbach, H. (1937). Beweis einer
zahlentheoretischen Ungleichung. J. Reine Angew. Math. 177:
193--196. DOI: 10.1515/crll.1937.177.193

\bibitem{TaoRogers} Tao, T. (2026). Rogers' theorem on sieving.
Blog post, 19 January 2026. Available at:
\url{https://terrytao.wordpress.com/2026/01/19/rogers-theorem-on-sieving/}

\bibitem{TaoThread} Tao, T. (2026). Comment of 18 January 2026 on
Erd\H{o}s Problem 281, quoting G.~Tenenbaum with permission.
Available at:
\url{https://www.erdosproblems.com/forum/thread/281#post-3370}

\bibitem{Tenenbaum} Tenenbaum, G. (1995). \emph{Introduction to
Analytic and Probabilistic Number Theory}. Cambridge Studies in
Advanced Mathematics, Vol.~46. Cambridge, UK: Cambridge University
Press.

\bibitem{Ten96} Tenenbaum, G. (1996). On block Behrend sequences.
Math. Proc. Cambridge Philos. Soc. 120: 355--367. DOI:
10.1017/S0305004100074910

\bibitem{Williams} Williams, D. (1991). \emph{Probability with
Martingales}. Cambridge, UK: Cambridge University Press. DOI:
...
{\sloppy
\paragraph{Provenance of the illustrative measurements.}
...
chain constraint by the program \path{verify_organizable.py};
the saturation reduction by \path{verify_saturation.py}; the
...
eight-block drift sum of Section~\ref{sec:reduction} by
\path{verify_cylinder_thm.py}; the spike display of
Section~\ref{sec:routes}, the exact limits of Remark~\ref{rem:gap}
and the block measures of the excursion remark by the claims
script;
and the catalog audit of the eleven exhibited systems by
...
from pools disjoint by depth and coprime to $2,5,17$, nodes of large drift in proportion $1/k^2$ -- has $617$ nodes below the root and $23{,}361$
...
misattribution, eleven finite instances -- one or two for each
of Theorems~\ref{thm:transl}, \ref{thm:conv}, \ref{thm:H},
\ref{thm:D}, \ref{thm:bounded}, \ref{thm:exhaust},
\ref{thm:quasi}, \ref{thm:confined}, \ref{thm:tree}
and~\ref{thm:overlap}, some of them the paper's own systems and
some auxiliary -- were audited mechanically: for each, the
finitely checkable hypotheses of the theorem named for it are
verified on a truncation, the infinitary ones -- divergence,
distinctness, escape -- being established in the text where each
system is introduced; the finite checks pass.  The instance of
Theorem~\ref{thm:coprot} is checked separately by
\path{verify_coprime_protectors.py} (distinct attached primes,
the product formula, the split).  The saturation reduction of
