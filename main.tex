...
logarithmic density of $A$ is at most $\nu$, so that the problem
is the reverse inequality.  We prove the reverse inequality in
...
each under explicit hypotheses on the layers.
...
maximal holes of its survivor set.  What remains is stated as
...
\noindent\textit{2020 Mathematics Subject Classification.} 11B05, 11N35.\\
\textit{Keywords.} Sets of multiples, logarithmic density,
...
$\varepsilon$?  It was settled affirmatively in 2026 by Rogers'
theorem, which reduces the density to the aligned case, together
with the 1936 Davenport--Erd\H{o}s result there; it does not bear
on the existence of the density.  The
only prior progress on Problem~25 itself that we know of is a
2026 note of Chojecki~\cite{Choj26}, which proves the two
cases of convergent drift and pairwise coprime moduli --
Theorems~\ref{thm:conv} and~\ref{thm:D} below -- and reduces the
general problem to a uniform harmonic estimate for the finite
``quotient sieves'' by which each class is first reached, noting
as obstructions that the tail union admits no vanishing
logarithmic bound and that prime-power towers create transient
spikes; those two obstructions are, respectively, the failure of
the union bound under divergent drift and
Proposition~\ref{prop:spike} of this paper.  Apart from those two
...
lies in $(n^{1-\varepsilon},n]$ be rare -- is the multiples-set
form of the top-octave phenomenon that Proposition~\ref{prop:spike}
controls here.  The theory of Behrend sequences -- sets of
...
$(1-\log2)/\bigl(1-(1+\log\log2)/\log2\bigr)\approx3.57$
measures the dependence among the events; the method conditions on the product $n_k$ of an
...
($\nu_c$, $\nu_C$), a set of classes or groups ($\nu_T$,
$\nu_J$) -- and a prime marks a reduced system.  A system is a
\emph{set} of classes: no ball is listed twice,
...
the integer side they enter in exactly four ways.  A finite
...
shape with a larger constant.  Every result of this paper is
stated for standard systems and holds for threshold systems with
the same proof, thresholds entering only through these four
facts: Theorems~\ref{thm:upper} and~\ref{thm:D} through the
first, Theorem~\ref{thm:conv} and Lemma~\ref{lem:tail} through
the second, Theorem~\ref{thm:bounded} through the third, and
Theorems~\ref{thm:exhaust}, \ref{thm:confined} and
Lemma~\ref{lem:light} through the fourth, with the system's
constant $M'$ in place of $1$.  Induced systems arise in two
...
constant.  No argument restricts repeatedly with unbounded
moduli: the reduction theorems of
Section~\ref{sec:reductions} act on the original system, and the
fourth fact bounds their tail kills by the original thresholds.
...
The whole content of Erd\H{o}s's question is therefore the
reverse inequality
...
into $\sum_{n\le X}\varphi(n)/n\sim L\log X$, the stated
logarithmic mean.  Note that
...
arbitrary they agree, and by Theorem~\ref{thm:upper} the common
value is $\nu$.
\end{proof}
...
$n_i$, so there are at most $X/n_i$ of them; disjointness then
gives $\bigl|\bigcup_i K_i\cap[1,X]\bigr|\le X\sum_i 1/n_i$,
...
The many-protector systems of Section~\ref{sec:reductions} fall on either side
according to the sparsity of the protectors: a class $p_jm$ has
smallest prime $\min(p_j,p(m))$, so $\Kh_p$ collects only the
protectors $p_j\ge p$ and $\sum_p\mu(\Kh_p)\approx\sum_j\pi(p_j)/p_j$,
convergent for exponentially sparse $p_j$ and divergent for
polynomially sparse ones, where positive survivor measure then
...
of residue classes, $S_N$ is periodic and has a density $d_N$
...
$(14,6)$, $(15,5)$, $(21,7)$ with $M=105$, at $X=2\cdot10^5$ one
finds $F(X)=0.2812$, with $F$ the harmonic mass of the killed
set defined in Section~\ref{sec:reduction}, against a logarithmic mean of $\varphi$
equal to $0.2871$.  What makes the proof above work is that the
...
$M'=\operatorname{lcm}(M,w_{i_1},\dots,w_{i_N})$, which is
...
$\dlog(S)=0$; note that natural density $0$ would \emph{not}
suffice here, since it does not imply vanishing logarithmic
density.  In the second case Theorem~\ref{thm:bounded} applies and gives
...
relaxation is reachable where the strict form is not: a fixed
finite obstruction can touch every cylinder for small $M$ and
only a vanishing share as $M$ grows, since a class of modulus
$n$ meets $M/\gcd(n,M)$ of the $M$ residues.  What the relaxed
hypothesis still demands, and what remains unproved in general,
is that the unresolved share can be driven to $0$ at all.
\end{remark}
...
The theorem is not vacuous, and covers a system the earlier
...
$C_j=c_j+d_j\mathbb{Z}$, and suppose the induced system of
...
$\min(d_j,\min_{i\in G_j}t_i)$, at most $d_j$ and at least
$d_j/M'$, so that the reduced system is a threshold system with
the same constant.  Since group $j$ kills only inside $C_j$ and only
from its thresholds on, $U\subseteq U'$ exactly, so
...
$\sum1/p_j<\infty$, and let $G_j$ consist of the moduli $p_jm$
with residue $\equiv1\pmod{p_j}$ and residues in general
position modulo $m$.  Then $G_j$ kills only inside
$C_j=\{x\equiv1\ (p_j)\}$, and within it acts as a full system
with residues in general position, which dies.  The moduli have
greatest common divisor $1$ and admit no finite prime cover,
since $p_j$ itself is a modulus of $G_j$; the residues are
misaligned; each group's drift diverges; and yet
$\nu=\prod_j(1-1/p_j)>0$.  With
$p_j\in\{11,197,439,691,977,1259\}$ the measured survivor
density is $0.8995$, agreeing with the product to four
decimals, each cylinder's survivor share is $0.0000$, and the
complement's is $1.0000$.  So positive survivor measure against
...
primes $q_i$; for each, level-one classes $(q_im,c)$ with
...
$\prod(1-1/q_i)$ to four decimals, every level-one cylinder
...
Let $\mathcal{P}$ be the family of cylinders $C$ with
...
\begin{proof}
A class $B_i$ has $\mu(\Ahat\cap B_i)=0$, and the cylinders
containing it with null intersection form a family in which each
coarsening divides the modulus, hence finite, so a maximal one
exists.  Every member of $\mathcal{P}$ lies in the union of the
balls up to a null set, meeting $\Ahat$ in a null set, which
gives the union and the measure.  Saturation is maximality read
twice, and since the profinite survivor set of $\mathcal{P}$ is $\Ahat$ up to a
null set and $\mathcal{P}$ is determined by $\Ahat$ up to null sets, the reduction is
idempotent.  For the density, a class inside $P\in\mathcal{P}$
...
original exactly, $A\supseteq A_{\mathcal{P}}$, and
...
passes down.  The consequence is that in attacking the problem
one may assume the system saturated: its classes are the maximal
holes of its survivor set, no proper coarsening is null, and any
cylinder that dies is already inside a class.  The
...
In the saturated form the classes are the holes of $A$, and
...
level $P$, into escapees of summable measure -- the hypothesis
of Theorem~\ref{thm:confined} -- and classes anchored to a
bounded family of prime cylinders, or its escapees are
...
measure lies in an enormous number of holes.  That is the only
place a counterexample can live.  We have not been able to convert
such concentration into one of the organisations covered above.
\end{remark}
...
$\le\nu$.  The groups beyond $J$ kill only inside their cylinders, and
only integers at least $d_j/M'$ by the fourth threshold fact
($M'=1$ for a standard system); so
...
fix $\eta,\varepsilon>0$.  Let $\mathcal{F}_D$ be the finite
...
Let $C$ be an alive cell, a residue class.  The classes
attached to $C$'s node, to its
ancestors and to its descendants form, in the index $t$, a
coprime-layered system with the same branch drift -- for a gap
there are no descendants, and (T4) keeps the attached primes
coprime to the modulus of $C$ -- whose survivor set is the index
set of $A\cap C$ and whose survivor measure is
$\nu_C=\mu(C\cap\{\text{survivors}\})/\mu(C)$.
...
cylinders any two of which share a factor: the moduli
$2,10,170,\dots$ of the tree above, not the moduli
$11,23,43,\dots$ of a family of protectors.
...
over the legal node assignments, the depth-four tree above and
the multiples of $6$ are coprime-layered, while those two
systems are not.
\end{remark}
...
single cylinder $\mathbb{Z}$, with composite moduli, divergent
...
$\mu(\Kh_{pp'})=\kappa_{pp'}/(pp')$.  Discarding the pairs
meeting the primes at most $P$, and the finitely many groups
whose attached primes include one at most $P$ -- finitely many
because each prime is attached to at most one group -- leaves a
family determined by the coordinates above $P$, whose reciprocal
sum still diverges, enumerated by the pairs among the first $N$
...
primes, total protector measure $0.1191$ and survivor measure
$0.9070$ against a Monte Carlo value of $0.9044$: there the sums
...
classes, $\sigma(X)\le 2N/X$.  Each of these classes
contributes at least $1/X$ to the window drift
$d(X)=\sum_{X/2<n_i\le X}1/n_i$, giving $N/X\le d(X)$ and
therefore $\sigma(X)\le2d(X)$.
...
\le(\log2)\,D(Y),
\]
so Markov's inequality for the measure $dX/X$ on $[1,Y]$ bounds
$\int_{\{X\le Y:\ 2d(X)>\delta\}}dX/X$ by $(2\log2)D(Y)/\delta$,
whose ratio to $\log Y$ has limsup $(2\log2)h/\delta$; and
$\sigma\le2d$.
\end{proof}
...
exists and equals $\nu$ as soon as $r\to1$ in logarithmic mean.
\end{proposition}
...
nine dyadic windows up to $4\cdot10^6$, a modulus of size
...
Second, the deviation is one-sided.  For each $N$ the survivors
of the whole system lie inside those of the first $N$ classes,
whose window density tends to $\nu_N$; hence
$\limsup_X s(X)\le\nu$ and so $\limsup_X r(X)\le1$.  Combining,
...
system of all moduli with residues in general position, where
$\nu=0$, it diverges on every cylinder.
\end{remark}
...
a coprime subfamily could have at most that many members.  Both
...
\begin{enumerate}
\item[(i)] A class of level-zero modulus $n_0$ persists for at
most $\log_2 n_0$ levels, since each proper coarsening passes to
a proper divisor and at least halves the modulus; at level $K$
only classes with $n_0\ge2^K$ remain.
\item[(ii)] No level contains a modulus equal to $1$, for that
cylinder is all of $\mathbb{Z}$ and its exhaustion forces
$\nu=0$.
...
classes with unboundedly long divisor chains, each level of
...
settles it.  Over twenty sparse primes, $\nu=0.9535$
with drift $0.093$ at every level; over four hundred dense
primes, drift $2.33$ with $\nu$ at $0.29$ and falling to $0$ as
primes are added.  The two conditions were not obtainable
together.
...
the least prime exceeding both $p_{j-1}$ and $8j^{3/2}$, and
$S_j=\sqrt j$ -- and let
...
measure $0$, so Theorem~\ref{thm:exhaust} admits no proper
...
cylinder holding two classes has prime modulus: for $(p_j,1)$
...
With six groups the system has $5{,}964$ classes; every
cylinder holding two or more classes has prime modulus and
distinct prime induced moduli, the least induced survivor
measure is $0.0633$, the largest induced drift $2.449=\sqrt6$,
$\sum_{j\le6}1/p_j=0.1908$, and $5{,}951$ classes are
...
hopes are therefore closed off.  Every system does \emph{not}
admit a coprime-layered refinement -- the protector systems and
...
nothing; the semiprimes, for instance, are covered by
Theorem~\ref{thm:quasi} rather than by any grouping.  That is
...
\item on some cylinder, no pairwise coprime subfamily of
divergent induced drift (Corollary~\ref{cor:dense});
...
(Theorem~\ref{thm:exhaust}); and, by Theorem~\ref{thm:sat}, the
same for its saturated form, so that the system may be taken
saturated;
...
$\sim\tfrac12\log X$, and no alignment.  Its survivors decay to
...
survivors.  In its first form the Question asked whether positive survivor
...
for this test: its drift grows like $s\log\log$ and reaches only
$0.05$ over eight small primes, so neither the protected nor the
...
Theorem~\ref{thm:exhaust}.  If instead
$\nu>0$ then $\sum_j d_{N_j}<\infty$, so $d_{N_j}\to0$ and the
bound vanishes for heads beyond $j$.  Either way the excursion
cannot be sustained.  We do not claim this settles necessity,
only that the natural counterexample strategy does not work.
\end{remark}
...
not work.  These appear to us more useful to a reader attacking
the problem than the positive results.  The last is an attempt
to \emph{refute} the conjecture rather than prove it.
...
$q\in Q$, the cutoff large enough that every block holds at
least $q$ primes; take as moduli the products $qp$ for $p\in P_q$, list each
...
\[
  ...
  \sum_q\frac{1}{q\,\min P_q}\ +\ s\,P(2),
\]
where $P(2)=\sum_p p^{-2}$ is the prime zeta value.  Both terms
converge, the first because consecutive blocks of fixed harmonic
weight $s$ force, by Mertens' theorem, the least prime of the
$k$th block to exceed $\exp(e^{ks})$ up to a constant, so that
$\sum_q1/(q\min P_q)$ is dominated by a doubly exponentially
decaying series.  The
total drift, meanwhile, is $\sum_q s/q$, which diverges by
Mertens.  (The cruder bound $s\,P(2)$ alone is not correct: the
block at $q=2$ contributes about $s/4$ on its own.)
...
computing each survivor density on the window
$[X/2,X]$ with $X=6\cdot10^4$ -- a window, not the exact periodic
density, so these figures are indicative -- $56.1\%$ fall below
the bound, and the displayed system, for which the periodic
density $1/8$ is exact, is the extreme case.  The enumeration
is stated here in full so that it may be reproduced directly.

\subsection*{Isolated transient spikes cannot make the logarithmic
average oscillate}
...
Taking the class $0\bmod n$ for every $n\in(Y/2,Y]$, at
$Y=2^{13}$, $2^{16}$, $2^{19}$, $2^{21}$, the killed density of
the dyadic window $(2^k,2^{k+1}]$ runs, for $k=12,\dots,20$,
through
\[
...
which case the pseudo-random seed is fixed in the program; each
check is accompanied by a control designed to fail if the
property being checked were absent.  The values on which the
...
Remark~\ref{rem:gap} to show why two plausible routes fail.  The programs, their outputs and this paper form a self-contained
artifact that accompanies the manuscript; a versioned public
copy will be deposited on publication.
...
of Section~\ref{sec:reductions} by \texttt{verify\_multiprotector.py} and
\texttt{verify\_tower.py}; the window ratios and the
intermediate-range ratios by \texttt{verify\_buchstab.py} and
\texttt{verify\_overkill.py}; the induced-drift measurements of
the refinement remark by \texttt{verify\_drift\_invariance.py};
the coarsening statistics by \texttt{verify\_coarsening.py}; the
semiprime, protector, tree and overlapping-protector instances
by \texttt{verify\_quasi.py}, \texttt{verify\_confined.py},
\texttt{verify\_coprime\_protectors.py}, \texttt{verify\_tree.py}
and \texttt{verify\_overlap.py}; the node assignments of the
chain constraint by \texttt{verify\_organizable.py}; the
saturation reduction by \texttt{verify\_saturation.py}; the
spike and excursion sequences of Section~\ref{sec:routes} by
\texttt{verify\_spike\_bound.py} and \texttt{verify\_excursion2.py};
and the coverage table by \texttt{verify\_coverage.py}.  The
values that enter hypotheses or explicit assertions are
re-derived by \texttt{tools/verify\_paper\_claims.py} in one run,
whose output is archived with the artifact.  The disjointness bound
of Lemma~\ref{lem:A} was confirmed against an exhaustive
branch-and-bound maximum over all $104$ classes of modulus between $2$ and
at most $14$, returning exactly $1.000000$.  The independence
...
exhaustively for four systems, confirming that the depth-four
tree and the multiples of $6$ are coprime-layered and that the
protector and semiprime systems are not.  For
...
three decimals at $X=10^4,10^5,10^6$ and $4\times10^6$, which
...
checked by hand; the finite checks pass.  The saturation reduction of
...
take full responsibility for the content.  Every numerical
assertion is reproduced by the artifact's programs.
